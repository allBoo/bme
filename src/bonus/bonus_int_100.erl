...
%%% ====================================================================
%%% Бонус за 100 интуиции
%%% ====================================================================


-module(bonus_int_100).
-behaviour(gen_buff).
-include_lib("bme.hrl").
...
new(Buff) ->
	?DBG("Start bonus_int_100 module~n", []),
	{ok, Buff#buff{
			id   = bonus_int_100,
			...
			time = infinity,
			charges = undefined,
			value = [{'user.dpower.crit', 25},
					 {'user.mfs.ucrit', 50},
					 {'user.mfs.acrit', 5}]
		}}.


on_unit_state_change({'user.stats.int', _}, Buff) ->
	Unit = unit:get_state(Buff#buff.unit),
	Int  = ?stats(?user(Unit))#u_stats.int,
	case Int of
		_125 when _125 >= 125 -> {swap, bonus_int_125, Buff};
